...
| <a name="input_asg_max_size"></a> [asg\_max\_size](#input\_asg\_max\_size) | Maximum number of instances in ASG. By default, the number of backend subnets plus one. | `number` | `null` | no |
| <a name="input_asg_min_elb_capacity"></a> [asg\_min\_elb\_capacity](#input\_asg\_min\_elb\_capacity) | Terraform will wait until this many EC2 instances in the autoscaling group become healthy. By default, it's equal to number of backend subnets. | `number` | `null` | no |
...
| <a name="input_nlb_healthcheck_healthy_threshold"></a> [nlb\_healthcheck\_healthy\_threshold](#input\_nlb\_healthcheck\_healthy\_threshold) | Number of times the host have to pass the test to be considered healthy | `number` | `5` | no |
